...
class TestRemoveCommentsStrings:
    ...
    def test_preserves_strings_with_quotes(self):
        ...
        code = [
            'string public name = "MyContract"; // comment'
        ]
        result = remove_comments_strings(code)
...
class TestGetPragmaContractNames:
    ...
    def test_extract_pragma_from_simple_contract(
    ...
    def test_extract_pragma_from_old_pragma(
        self, fixtures_dir
    ):
        """Test pragma extraction from old pragma contract."""
        old_pragma = (
            fixtures_dir / "contracts" / "OldPragma.sol"
        ).read_text()
        pragma, contracts = get_pragma_contractnames(
            [old_pragma]
        )
    ...
    def test_extract_pragma_from_new_pragma(
        self, fixtures_dir
    ):
        """Test pragma extraction from new pragma contract."""
        new_pragma = (
            fixtures_dir / "contracts" / "NewPragma.sol"
        ).read_text()
        pragma, contracts = get_pragma_contractnames(
            [new_pragma]
        )
    ...
    def test_no_pragma_returns_none(self, fixtures_dir):
        ...
        no_pragma = (
            fixtures_dir / "contracts" / "NoPragma.sol"
        ).read_text()
        pragma, contracts = get_pragma_contractnames(
            [no_pragma]
        )
    ...
    def test_extract_multiple_contracts(self, fixtures_dir):
        ...
        multi_contract = (
            fixtures_dir / "contracts" / "MultiContract.sol"
        ).read_text()
        pragma, contracts = get_pragma_contractnames(
            [multi_contract]
        )
    ...
    def test_extract_library_name(
        self, sample_library_contract
    ):
        """Test extraction of library name."""
        pragma, contracts = get_pragma_contractnames(
            [sample_library_contract]
        )
        assert pragma == "pragma solidity ^0.8.0;"
...
class TestEnsureSolcVersionsLoaded:
    ...
    @patch(
        "sb.solidity.solcx.get_installable_solc_versions"
    )
    def test_loads_installable_versions_successfully(
        self, mock_get_installable
    ):
        ...
        assert (
            sb.solidity.cached_solc_versions
            == mock_versions
        )
    ...
    def test_returns_cached_versions(
        self, mock_get_installable
    ):
    ...
    def test_fallback_to_installed_versions_on_error(
        ...
        mock_get_installable.side_effect = Exception(
            "Network error"
        )
...
class TestGetSolcVersion:
    ...
    @patch(
        "sb.solidity.solcx.install._select_pragma_version"
    )
    def test_selects_version_from_pragma(
        self, mock_select_version
    ):
    ...
    def test_transforms_gte_to_caret(
        self, mock_select_version
    ):
    ...
    def test_adds_patch_version_to_short_version(
        self, mock_select_version
    ):
    ...
    def test_handles_version_selection_error(
        self, mock_select_version
    ):
        """Test handling of version selection errors."""
        mock_select_version.side_effect = Exception(
            "Invalid pragma"
        )
        version = get_solc_version(
    ...
    def test_handles_complex_pragma_range(
        self, mock_select_version
    ):
...
class TestGetSolcPath:
    ...
    def test_installs_and_returns_path(
        self, mock_get_executable, mock_install
    ):
        ...
        mock_get_executable.assert_called_once_with(
            "0.8.20"
        )

    @patch("sb.solidity.solcx.install_solc")
    @patch("sb.solidity.solcx.get_executable")
    def test_returns_cached_path(
        self, mock_get_executable, mock_install
    ):
        """Test that cached path is returned without reinstalling."""
        import sb.solidity

        sb.solidity.cached_solc_paths["0.8.20"] = (
            "/cached/path/solc"
        )
    ...
    @patch("sb.solidity.solcx.install_solc")
    @patch("sb.solidity.solcx.get_executable")
    def test_handles_get_executable_error(
        self, mock_get_executable, mock_install
    ):
        """Test handling of get_executable errors."""
        mock_get_executable.side_effect = Exception(
    ...
    @patch("sb.solidity.solcx.install_solc")
    @patch("sb.solidity.solcx.get_executable")
    def test_handles_none_executable_path(
        self, mock_get_executable, mock_install
    ):
...
class TestIntegrationScenarios:
    ...
    def test_full_workflow_simple_contract(
        ...
        mock_select_version.return_value = "0.8.20"
        mock_get_executable.return_value = Path(
            "/usr/bin/solc-0.8.20"
        )
...
class TestEdgeCases:
    ...
    def test_version_transformation_with_multiple_ranges(
        self,
    ):
    ...
    @patch("sb.solidity.solcx.install_solc")
    @patch("sb.solidity.solcx.get_executable")
    def test_caching_works_across_multiple_calls(
        self, mock_get_executable, mock_install
    ):
