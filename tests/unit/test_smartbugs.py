...
class TestCollectFiles:
    ...
    def test_collect_single_sol_file(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_collect_multiple_sol_files_with_glob(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_collect_recursive_glob(
        self, tmp_path: Path
    ) -> None:
        ...
        (tmp_path / "Top.sol").write_text(
            "pragma solidity ^0.8.0;"
        )
        (tmp_path / "contracts" / "Middle.sol").write_text(
            "pragma solidity ^0.8.0;"
        )
        (subdir / "Deep.sol").write_text(
            "pragma solidity ^0.8.0;"
        )
    ...
    def test_collect_with_root_directory(
        ...
        try:
            ...
            assert (
                "colons in file patterns only supported"
                in str(e)
            )

    def test_collect_ignores_directories(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_collect_ignores_non_sol_hex_files(
        self, tmp_path: Path
    ) -> None:
        ...
        assert len(files) == 2
        extensions = [
            os.path.splitext(f[0])[1] for f in files
        ]
    ...
    def test_collect_nonexistent_file(
        self, tmp_path: Path
    ) -> None:
        """Test collecting nonexistent file returns empty list."""
        patterns = [
            (None, str(tmp_path / "nonexistent.sol"))
        ]
        files = sb.smartbugs.collect_files(patterns)

        assert files == []

    def test_collect_removes_duplicates(
        self, tmp_path: Path
    ) -> None:
...
class TestCollectTasks:
    ...
    def test_collect_tasks_basic(
        ...
        settings = Settings()
        settings.results = str(
            tmp_path / "results" / "${TOOL}" / "${FILENAME}"
        )
        settings.timeout = 60
    ...
    def test_collect_tasks_main_contract_not_found(
        ...
        settings.main = (
            True  # Expect contract named 'MyContract'
        )
    ...
    def test_collect_tasks_no_matching_compiler(
        ...
        mock_is_loaded.return_value = True
        mock_ensure_loaded.return_value = True
        mock_get_solc_version.return_value = (
            None  # No matching version
        )
        mock_read_lines.return_value = [
            "pragma solidity ^0.99.0;",
            "contract Test {}",
        ]
    ...
    def test_collect_tasks_continue_on_errors(
        ...
        files = [
            (str(bad_file), "Bad.sol"),
            (str(good_file), "Good.sol"),
        ]
        tool = self._create_test_tool(
            "test_tool", "solidity", solc=True
        )
    ...
    @staticmethod
    def _create_test_tool(
        tool_id: str, mode: str, solc: bool = False
    ) -> sb.tools.Tool:
...
class TestToolLoading:
    ...
    def test_load_single_tool(
        self, mock_read_yaml: MagicMock, tmp_path: Path
    ) -> None:
    ...
    @patch("sb.io.read_yaml")
    def test_load_prevents_duplicate_tools(
        self, mock_read_yaml: MagicMock, tmp_path: Path
    ) -> None:
...
class TestMain:
    ...
    def test_main_orchestration_flow(
        ...
        mock_collect_files.assert_called_once_with(
            [(None, "*.sol")]
        )
        ...
        mock_analysis_run.assert_called_once_with(
            [mock_task], settings
        )
