...
def parse(
    ...
    for f in list(
        fails
    ):  # make a copy of fails, so we can modify it
        if f.startswith(
            "exception (teether.evm.exceptions."
    ...
    for line in log:
        if line.startswith(
            ...
            infos.add(
                "No state-dependent critical path found"
            )
            analysis_completed = True
            ...
            if error.startswith("Failed path due to "):
                ...
                if e.startswith("b'Argument ") or any(
                    e in f for f in fails
                ):
                    continue
    ...
    if exploit:
        findings = [
            {"name": "Ether leak", "exploit": exploit}
        ]
