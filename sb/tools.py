...
class Tool:
    ...
    def __init__(self, cfg: dict[str, Any]) -> None:
        ...
        for k in ("id", "mode"):
            if not getattr(self, k):
                raise sb.errors.InternalError(
                    f"Tool: Field '{k}' missing.\n{cfg}"
                )
        if not self.image:
            raise sb.errors.SmartBugsError(
                f"Tool {self.id}/{self.mode}: no image specified"
            )
        ...
        if self.bin and self.id:
            self.absbin = os.path.join(
                sb.cfg.TOOLS_HOME, self.id, self.bin
            )
    ...
    def dict(self) -> dict[str, Any]:
        ...
        for k, v in self.__dict__.items():
            if k == "_command":
                d["command"] = (
                    self._command.template
                    if self._command
                    else None
                )
            elif k == "_entrypoint":
                d["entrypoint"] = (
                    self._entrypoint.template
                    if self._entrypoint
                    else None
                )
            elif k == "absbin":
    ...
    for id in ids:
        ...
        for mode in ("solidity", "bytecode", "runtime"):
            ...
            if not isinstance(cfg[mode], dict):
                raise sb.errors.SmartBugsError(
                    f"Tool {id}/{mode}: key/value mapping expected."
                )
