...
def get_parser(tool: dict[str, Any]) -> "ModuleType":
    ...
    if key not in tool_parsers:
        try:
            ...
            fn = os.path.join(
                sb.cfg.TOOLS_HOME, tid, tool["parser"]
            )
            ...
            raise sb.errors.SmartBugsError(
                f"Cannot load parser for {tid}/{tmode}\n{e}"
            )
...
def parse(
    ...
    try:
        findings, infos, errors, fails = tool_parser.parse(
            exit_code, tool_log, tool_output
        )
        for finding in findings:
            ...
            if (
                tool_parser.FINDINGS
                and finding["name"]
                not in tool_parser.FINDINGS
            ):
                raise sb.errors.SmartBugsError(
